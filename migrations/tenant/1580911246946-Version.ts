...
import { BcryptProvider } from '../../src/Core/Providers/bcrypt.provider';
...
export class Version1580911246946 implements MigrationInterface {
  ...
  private async populateUser(queryRunner: QueryRunner): Promise<number[]> {
    ...
    for (const user of userSeed) {
      ...
      tasks.push(
        queryRunner.query(`INSERT INTO users(name, username, email, password, is_visible, is_admin)
                                    VALUES ('${user.name}', '${user.username}', '${user.email}', '${user.password}',
                                            ${user.isVisible}, ${user.isAdmin});`),
      );
    }

    return Promise.all(tasks).then((result) =>
      result.map((insertUser) => insertUser.insertId),
    );
  }

  private async populateTranslationStatus(
    ...
    return queryRunner.query(
      'INSERT INTO translation_status(status) VALUES ' + inserts.join(', '),
    );
  }
}
