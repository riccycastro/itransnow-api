...
describe('TranslationStatusService', () => {
  ...
  beforeAll(async () => {
    ...
    translationStatusRepository = app.get<TranslationStatusRepository>(TranslationStatusRepository);
  });
  ...
  describe('getByStatus', () => {
    it('should throw a not found exception error', async () => {
      const findOneSpy = jest.spyOn(translationStatusRepository, 'findOne').mockImplementation(async () => {
        return undefined;
      });

      await expect(translationStatusService.getByStatus('status_alias')).rejects.toThrow(NotFoundException);
    ...
    it('should return a translation status', async () => {
      ...
      expect(await translationStatusService.getByStatus('status_alias')).toEqual(buildTranslationStatusWithId1());
      expect(findOneSpy).toHaveBeenCalledTimes(1);
    });
  });
});
