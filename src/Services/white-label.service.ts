...
@Injectable()
export class WhiteLabelService extends AbstractEntityListingService<
  WhiteLabel
> {
  ...
  async findByAliasOrFail(
    ...
    const whiteLabel = await this.getByAlias(applicationId, alias);
  ...
  private async getByAlias(
    ...
    return await (this.repository as WhiteLabelRepository).findByAlias(
      applicationId,
      alias,
    );
  }
  ...
  async getByApplication(
    ...
    return await (this.repository as WhiteLabelRepository).findByApplication(
      companyId,
      applicationId,
      query,
    );
  ...
  protected async getEntityListAndCount(
    ...
    const listResult = await (this
      .repository as WhiteLabelRepository).findInList(applicationId, query);

    for await (let whiteLabel of listResult[0]) {
  ...
  async create(
    ...
    const whiteLabelAlias = await this.validateAlias(
      whiteLabelDto,
      application,
    );
  ...
  active(
    ...
    whiteLabelDto: ActiveWhiteLabelDto,
  ): WhiteLabel {
  ...
  async createWhiteLabelTranslation(
    user: User,
    whiteLabel: WhiteLabel,
    ...
    const language = await this.languageService.getByCodeInApplication(
      whiteLabel.applicationId,
      whiteLabelTranslationDto.language,
    );
    const translationStatus = await this.translationStatusService.getByStatus(
      TranslationStatusService.APPROVAL_PENDING,
    );
    const translationKey = await this.translationKeyService.getByTranslationKeyInApplication(
      user.companyId,
      whiteLabel.applicationId,
      whiteLabelTranslationDto.translationKey,
    );

    const translation = this.translationService.create(
      language,
      user,
      translationStatus,
      whiteLabelTranslationDto.translation,
    );
    ...
    try {
      ...
      whiteLabelTranslation.translation = await this.translationService.save(
        translation,
        queryRunner.manager,
      );
      ...
      whiteLabelTranslation = await queryRunner.manager.save(
        whiteLabelTranslation,
      );
  ...
  async getIncludes(
    ...
    if (query.includes.includes('application')) {
      whiteLabel.application = await this.applicationService.getById(
        whiteLabel.applicationId,
      );
    }
  ...
  async validateAlias(
    ...
    const whiteLabelAlias = this.stringProvider.removeDiacritics(
      whiteLabelDto.alias,
    );
    ...
    if (
      ...
      throw new BadRequestException(
        `White label with alias "${whiteLabelAlias}" already exists in ${application.name} application`,
      );
    }
