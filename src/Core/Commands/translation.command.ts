...
export class TranslationCommand {
  ...
  //   const application = await this.applicationService.getByAliasOrFail(
  //     company.id,
  //     applicationAlias,
  //   );
  ...
  //     console.log(
  //       `Use the api to add at least one language to ${applicationAlias} application`,
  //     );
  //     return;
  //   }
  //
  //   const languages = await this.languageService.getByIds(
  //     application.languagesId,
  //   );
  //
  //   for (let i = 0; i < amount; i++) {
  //     const translationKey = this.stringProvider.removeDiacritics(
  //       faker.random.words(),
  //     );
  //
  //     try {
  //       // this is to avoid repeated translation keys
  //       await this.translationKeyService.getByTranslationKeyInApplication(
  //         company.id,
  //         application.id,
  //         translationKey,
  //       );
